	...
	<?php foreach ($stock_almacenes as $id_alm => $datos): ?>
		...
		<tr>
			...
			<?php foreach ($datos['actuaciones'] as $dia_act => $valor): ?>
				<?php if ($valor): ?>
					<?php $valor_desplegar = (set_value('dato') === 'monto') ? fmtMonto($valor, 'MM') : fmtCantidad($valor); ?>
	...
	<tfoot>
		<tr class="active">
			...
			<?php foreach ($tot_col as $dia_act => $valor): ?>
				<th class="text-center"><?= set_value('dato') === 'monto' ? fmtMonto($valor, 'MM') : fmtCantidad($valor);  ?></th>
			<?php endforeach; ?>
		</tr>
	</tfoot>
</table>
